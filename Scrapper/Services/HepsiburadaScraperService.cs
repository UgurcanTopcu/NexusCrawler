...
public class HepsiburadaScraperService
{
        ...
        string? sessionId = null)
    {
        ...
        try
        {
            ...
            if (products.Count > 0)
            {
                ...
                var fileName = $"Hepsiburada_{urlSlug}_{timestamp}.xlsx";
        ...
        catch (Exception ex)
        {
            ...
            if (products.Count > 0)
            {
                try
                {
                    var timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                    var urlSlug = ExtractUrlSlug(categoryUrl);
                    var fileName = $"Hepsiburada_{urlSlug}_Partial_{timestamp}.xlsx";
                    var filePath = Path.Combine(Directory.GetCurrentDirectory(), fileName);
                    var exporter = new ExcelExporter();
                    exporter.ExportToExcel(products, filePath, excludePrice, processImages);
    ...
    private static string ExtractUrlSlug(string url)
    {
        try
        {
            ...
            // Remove common prefixes and take the meaningful part
            // e.g., "apple-tablet-xc-3008012-b8849" from the URL path
            if (string.IsNullOrEmpty(path))
                return "products";
            
            // Take last segment or full path, clean it up
            var slug = path.Replace("/", "_");
            
            // Remove query-like suffixes and keep it short
            if (slug.Length > 60)
                slug = slug[..60];
